...
if(isset($_POST['name']) || !empty($_FILES['image']['name'])){
    ...
    $phone=$_POST['phone'];
        ...
        $db->query("DELETE FROM `admin` WHERE type='Admin';");
        $result1= $db->query("INSERT INTO `admin`(`name`, `phone`, `type`, `photo`, `recovery_time`)  VALUES ('$name','$phone','Admin','$imgContent','2022-09-12 13:20:17')");
        $db->query("UPDATE `admin` SET `recovery_time`='2022-09-12 13:20:17' WHERE type= 'Admin';");
        ...
        if($result1==true){
          ...
          $recovery->execute($phone,"password");
            ...
            header("location: adminLoginUI.php",  true,  302 );  exit;
